...
OpenTelemetry separates instrumentation from the backend. An application creates traces, metrics, or logs, then exports them to a collector or backend. OpenTelemetry defines the APIs, protocol, and conventions, but it does not provide the storage and investigation UI itself.[^opentelemetry]
...
Langfuse accepts traces through an OTLP endpoint. Direct OTLP ingestion uses project API keys through a Basic Authentication header. For self-hosting, the documented architecture includes two application containers:[^langfuse]
...
Phoenix is still the easiest of these three for me to picture as one local unit. Its self-hosted server includes both a web UI and an OTLP trace collector. The configuration reference documents OTLP over HTTP and gRPC.[^phoenix] It can start with SQLite for a small local setup or connect to PostgreSQL for a more durable deployment.
...
LLM observability also needs a content boundary. A trace may contain prompts, system instructions, tool arguments, command output, file contents, and repository information. Exporting the trace successfully does not mean every field should be captured. Work such as filtering repository telemetry through content exclusion made that boundary explicit.[^content-controls] Content exclusion, bounded payloads, and opt-in capture need to be considered across the whole path.

The public VS Code agent monitoring guide shows how to enable collection and send the resulting OpenTelemetry signals to a compatible backend.[^vscode-guide]
...
[^opentelemetry]: OpenTelemetry, [What is OpenTelemetry?](https://opentelemetry.io/docs/what-is-opentelemetry/).
[^langfuse]: Langfuse documentation for [OpenTelemetry ingestion](https://langfuse.com/docs/integrations/opentelemetry) and [self-hosting architecture](https://langfuse.com/self-hosting).
[^phoenix]: Arize Phoenix, [Self-hosting configuration](https://arize.com/docs/phoenix/self-hosting/configuration).
[^azure-monitor]: Microsoft Learn, [Enable Azure Monitor OpenTelemetry](https://learn.microsoft.com/en-us/azure/azure-monitor/app/opentelemetry-enable).
[^vscode-work]: Implementation references: [GenAI instrumentation](https://github.com/microsoft/vscode-copilot-chat/pull/3917), [local SQLite storage and OTLP forwarding](https://github.com/microsoft/vscode/pull/316338), [enterprise OpenTelemetry settings](https://github.com/microsoft/vscode/pull/323227), and [Agent Host telemetry and trace context](https://github.com/microsoft/vscode/pull/328529).
[^content-controls]: VS Code, [filter repository telemetry through content exclusion](https://github.com/microsoft/vscode/pull/328791).
[^vscode-guide]: VS Code documentation, [Agent monitoring](https://code.visualstudio.com/docs/agents/guides/monitoring-agents).
